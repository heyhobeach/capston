...
//허스키렌즈에 ID값 학습되지 않은 태그 인식 시 정지(팔로우)
    if(return_cm2 <= 50 || husky_1_return_id == 0)
    {
     car_stop();
    }
...
void call_cart_move() {
    ...
    {
      car_forward();
      if(husky_2_return_id == 1) // 첫 번째 좌석
      {
        car_stop();
      }
    }
    ...
    {
      if(husky_2_return_id == 2) // 두 번째 좌석
      {
        car_stop();
      }
    }
...
void return_cart_move() {
  if(toSend == '7')
    {
      car_backward();
      if(husky_2_return_id == 3) // 첫 번째 좌석
      {
        car_stop();
      }
    }
...
void test_forward() {
    ...
    switch(a) {
     ...
     if (state == LOW){//LED가 꺼진상태면 
      forward_interval=2000; 
      state = HIGH;
     }//LED를 켜진 상태로 하고   
     else{//LED가 켜진상태라면
      forward_interval= 1200;   
      state = LOW; //LED를 꺼진상태로 한다.
     }
     //husky_1_return_id = 0;     
     //ledState변수에 저장된 값으로 ledPin핀의 상태를 설정한다   
     ...
     if (state == LOW){//LED가 꺼진상태면 
      forward_interval=1600; 
      state = HIGH;
     }//LED를 켜진 상태로 하고   
     else{//LED가 켜진상태라면
      forward_interval= 1400;   
      state = LOW; //LED를 꺼진상태로 한다.
     }
     //husky_1_return_id = 0;     
     //ledState변수에 저장된 값으로 ledPin핀의 상태를 설정한다   
